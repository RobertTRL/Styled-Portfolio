...
const Loader = ({ isDark, isLoading }) => {
  ...
  useEffect(() => {
    // When loading finishes, wait 1.5 seconds for the animation to play, then unmount
    if (!isLoading) {
  ...
  // Completely remove from DOM only after the timer finishes
  if (!shouldRender) return null;

  return (
    // Add the "water-dissolve" class when isLoading becomes false
