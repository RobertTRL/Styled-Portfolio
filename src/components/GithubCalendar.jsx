...
// Maps a contribution level + schema to a CSS class name
function getLevelClass(level, schema = "green") {
    const map = {
        NONE:           `gc-${schema}-0`,
        FIRST_QUARTILE: `gc-${schema}-1`,
        SECOND_QUARTILE:`gc-${schema}-2`,
        THIRD_QUARTILE: `gc-${schema}-3`,
        FOURTH_QUARTILE:`gc-${schema}-4`,
    }
    return map[level] ?? `gc-${schema}-0`
}
            ...
            {showTotal && (
                <div className="gc-header">
                    <div className="gc-username">
                        ...
                        <span className="gc-username-text">@{username}</span>
                    </div>
